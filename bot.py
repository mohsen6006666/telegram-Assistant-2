...
from config import TOKEN, logger, MAX_FILE_SIZE
...
class TelegramBot:
    ...
    def process_message(self, message):
        ...
        # For YTS.mx, we'll just use the search query directly
        # We can use format like: "720p Avengers" to search for 720p quality
    ...
    def _handle_file_selection(self, call, user_id: int) -> None:
        ...
        try:
            ...
            logger.error(traceback.format_exc())
            self.bot.edit_message_text(
                f"❌ Error downloading torrent file: {str(e)}\n\n"
                f"Please try another movie or search again.",
                chat_id=call.message.chat.id,
                message_id=call.message.message_id
            )
